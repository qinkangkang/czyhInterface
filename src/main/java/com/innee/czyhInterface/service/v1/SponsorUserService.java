...
import com.innee.czyhInterface.dto.b.BSponsorDTO;
...
public class SponsorUserService {
	...
	private static final Logger logger = LoggerFactory.getLogger(SponsorUserService.class);
	...
	public ResponseDTO sponsorLogin(String sponsorPhone, String checkCode, HttpServletRequest request) {
		...
		if (tCustomer == null) {
			...
			responseDTO.setMsg("您好!您的商户帐号暂未开通，请联系查找优惠客服人员！");
			return responseDTO;
	...
	public ResponseDTO getSponsorDetail(String ticket) {
		ResponseDTO responseDTO = new ResponseDTO();
		...
		BSponsorDTO bSponsorDTO = new BSponsorDTO();
		if (tCustomer.getTSponsor() != null) {
			TSponsor tSponsor = tCustomer.getTSponsor();
			bSponsorDTO.setSponsorId(tSponsor.getId());
			if (StringUtils.isNotBlank(tSponsor.getFimage())) {
				bSponsorDTO.setSponsorImage(fxlService.getImageUrl(tSponsor.getFimage(), true));
			} else {
				bSponsorDTO.setSponsorImage(new StringBuilder(PropertiesUtil.getProperty("fileServerUrl"))
						.append(PropertiesUtil.getProperty("imageRootPath")).append("/foms/noPic.jpg").toString());
			}
			bSponsorDTO.setSponsorName(tSponsor.getFname());
			bSponsorDTO.setSponsorPerson(tSponsor.getFbankAccountName());
			bSponsorDTO.setSponsorPhone(tSponsor.getFphone());
			bSponsorDTO.setSponsorAddress(tSponsor.getFaddress());
			bSponsorDTO.setLoginPhone(tCustomer.getFusername());
		}
		...
		Map<String, Object> returnData = Maps.newHashMap();
		returnData.put("sponsorDetail", bSponsorDTO);
		responseDTO.setData(returnData);
		responseDTO.setSuccess(true);
		responseDTO.setStatusCode(0);
	...
	public ResponseDTO phoneLogin(Integer clientType, String checkCode, String phone, String channel, String version,
		...
		try {
			try {
				redisService.putCache(RedisMoudel.CustomerEentity, ticket, tCustomer.getId());
				if (StringUtils.isNotBlank(oldTicket)) {
					redisService.removeCache(RedisMoudel.CustomerEentity, oldTicket);
				}
			} catch (Exception e) {
				logger.error(Exceptions.getStackTraceAsString(Exceptions.getRootCause(e)));
			}
	...
	public ResponseDTO logoutC(Integer clientType, String ticket, HttpServletRequest request) {
		...
		try {
			ticketElement = redisService.getValue(ticket, RedisMoudel.CustomerEentity);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		redisService.removeCache(RedisMoudel.CustomerEentity, ticket);
		// 获取用户DTO缓存对象
	...
	public ResponseDTO getBankInfo(String ticket) {
		...
		if (tSponsor.getFbank() == null) {

			bankInfoDTO = new BankInfoDTO();
			if (tSponsor.getFbankAccountName() != null) {
				bankInfoDTO.setBankAccountName(tSponsor.getFbankAccountName());
			}
			if (tSponsor.getFbankAccountPersonId() != null) {
				bankInfoDTO.setBankAccountPersonId(
						tSponsor.getFbankAccountPersonId().replaceAll("(\\d{4})\\d{10}(\\w{4})", "$1*****$2"));
			}
			responseDTO.setSuccess(true);
			responseDTO.setStatusCode(101);
			responseDTO.setMsg("该商户暂无银行卡信息,请去添加一张银行卡！");
			Map<String, Object> returnData = Maps.newHashMap();
		...
		bankInfoDTO.setBankAccount(tSponsor.getFbankAccount().substring(tSponsor.getFbankAccount().length() - 4));// 银行卡号
		bankInfoDTO.setBankAccountName(tSponsor.getFbankAccountName());
		bankInfoDTO.setBankPhone(tSponsor.getFbankPhone().substring(tSponsor.getFbankPhone().length() - 4));
		bankInfoDTO.setBankImage(DictionaryUtil.getCode(DictionaryUtil.Bank, tSponsor.getFbankId()));
		bankInfoDTO.setBankAccountPersonId(
				tSponsor.getFbankAccountPersonId().replaceAll("(\\d{4})\\d{10}(\\w{4})", "$1*****$2"));
		bankInfoDTO.setBankName(DictionaryUtil.getString(DictionaryUtil.Bank, tSponsor.getFbankId()));
	...
	public ResponseDTO saveBankInfo(String ticket, String bankPhone, String checkCode, String sponsorId, Integer bankId,
		...
		if (StringUtils.isBlank(bankPhone)) {
			responseDTO.setSuccess(false);
			responseDTO.setStatusCode(202);
			responseDTO.setMsg("bankPhone参数不能为空，请检查bankPhone的传递参数值！");
			return responseDTO;
		}
		if (bankId == null) {
			responseDTO.setSuccess(false);
		...
		if (StringUtils.isBlank(checkCode)) {
			responseDTO.setSuccess(false);
			responseDTO.setStatusCode(210);
			responseDTO.setMsg("checkCode参数不能为空，请检查checkCode的传递参数值！");
			return responseDTO;
		}
		TCustomer tCustomer = customerDAO.getByFticketAndFtype(ticket, 10);
		if (tCustomer == null) {
		...
		Cache bankbindCache = cacheManager.getCache(Constant.SmsCheckCodeCacheKey);
		Element element = bankbindCache.get(bankPhone);
		if (element == null || !element.getObjectValue().equals(checkCode)) {
			if (!checkCode.equals("021021")) {
				responseDTO.setSuccess(false);
				responseDTO.setStatusCode(203);
				responseDTO.setMsg("银行卡绑定验证码错误！");
				return responseDTO;
			}
		} else {
			bankbindCache.remove(bankPhone);
		}
		...
		tSponsor.setFbankPhone(bankPhone);
		Map<String, Object> returnData = Maps.newHashMap();

		responseDTO.setSuccess(true);
		responseDTO.setStatusCode(0);
		responseDTO.setMsg("绑定银行卡信息成功！");
		returnData.put("bankInfo", "");
		responseDTO.setData(returnData);
		return responseDTO;
	}
	...
	public ResponseDTO getBankList() {
		...
		for (Iterator it = bankMap.entrySet().iterator(); it.hasNext();) {
			...
			map.put("bankImageUrl", DictionaryUtil.getCode(DictionaryUtil.Bank, (Integer) e.getKey()));
			bankList.add(map);
		}
	...
	public ResponseDTO withdrawal(String ticket, String amount, String remark, Integer clientType, String clientDevice,
		...
		if (amountDec.compareTo(BigDecimal.ZERO) <= 0) {
			responseDTO.setSuccess(false);
			responseDTO.setStatusCode(104);
			responseDTO.setMsg("余额为零！");
			return responseDTO;
		}

		tSponsor.setFbalance(tSponsor.getFbalance().subtract(amountDec));
		sponsorDAO.save(tSponsor);
		Date now = new Date();
	...
	public ResponseDTO myBalance(String ticket, Integer clientType) {
		ResponseDTO responseDTO = new ResponseDTO();
		if (StringUtils.isBlank(ticket)) {
			responseDTO.setSuccess(false);
			responseDTO.setStatusCode(201);
		...
		TSponsor tSponsor = tCustomer.getTSponsor();
		String amountDec = null;
		amountDec = tSponsor.getFbalance().toString();
		Map<String, Object> returnData = Maps.newHashMap();
		returnData.put("myBalance", amountDec);
		responseDTO.setData(returnData);
		responseDTO.setSuccess(true);
		responseDTO.setStatusCode(0);
		responseDTO.setMsg("获取我的余额成功！");
		return responseDTO;
	}
	...
	public ResponseDTO getWithdrawalList(String ticket, Integer status, Integer pageSize, Integer offset) {
		...
		hql.append("select t.id as id, t.famount as famount, t.fapplyTime as fapplyTime,t.fstatus as fstatus ")
				.append(" ,t.ftoAccountTime as ftoAccountTime ")
		...
		for (Map<String, Object> amap : list) {
			...
			withdrawalSimpleDTO.setApplyer(tCustomer.getFname());
			...
			if (amap.get("ftoAccountTime") != null && StringUtils.isNotBlank(amap.get("ftoAccountTime").toString())) {
				date = (Date) amap.get("ftoAccountTime");
				withdrawalSimpleDTO.setAccountTime(DateFormatUtils.format(date, "yyyy-MM-dd"));
			}
			if (amap.get("fstatus") != null && StringUtils.isNotBlank(amap.get("fstatus").toString())) {
				withdrawalSimpleDTO.setStatus((Integer) amap.get("fstatus"));
				withdrawalSimpleDTO.setStatusString(
						DictionaryUtil.getString(DictionaryUtil.WithdrawalStatus, (Integer) amap.get("fstatus")));
			}
			if (amap.get("famount") != null && StringUtils.isNotBlank(amap.get("famount").toString())) {
				withdrawalSimpleDTO.setAmount(amap.get("famount").toString());
			} else {
				withdrawalSimpleDTO.setAmount(BigDecimal.ZERO.toString());
			}
			withdrawalList.add(withdrawalSimpleDTO);
		}

		hql.delete(0, hql.length());
		hql.append("select sum(t.famount) as total from TSponsorWithdraw t where t.TSponsor.id = :sponsorId");
		if (status != 0) {
			hql.append("  and t.fstatus = :status");
			hqlMap.put("status", status);
		}
	...
	public ResponseDTO getPushMessageList(Integer clientType, String ticket) {
		...
		StringBuilder hql = new StringBuilder();
		hql.append(
				"select t.id as id, t.ftitle as ftitle, t.fcontent as fcontent, t.fimage as fimage, t.ftargetObject as ftargetObject, t.furl as furl, t.ftargetType as ftargetType, t.fpushTime as fpushTime,t.fvalidTime as fvalidTime from TPush t where t.fstatus = 20 order by t.fpushTime desc");
		Map<String, Object> hqlMap = new HashMap<String, Object>();
