...
/**
 * Encapulated Java Cryptography Architecture endpoints <br />
 * 提供 RSA 的简便封装
 *
...
public class RSACrypto extends CryptoBase {
	...
	public RSAPublicKey generatePublic(byte[] m, byte[] e) {
		try {
			return ((RSAPublicKey) this.keyFac
					.generatePublic(new RSAPublicKeySpec(new BigInteger(1, m), new BigInteger(1, e))));
	...
	public RSAPrivateKey generatePrivate(byte[] m, byte[] d) {
		try {
			return ((RSAPrivateKey) this.keyFac
					.generatePublic(new RSAPrivateKeySpec(new BigInteger(1, m), new BigInteger(1, d))));
		} catch (InvalidKeySpecException ex) {
			throw (new RuntimeException(ex.getMessage(), ex));
		}
	}
