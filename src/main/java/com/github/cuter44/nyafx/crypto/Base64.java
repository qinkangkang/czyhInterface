...
public class Base64 {
	 ...
	 * 
	 * @param byteStream
	 *            In-param to receive result, usually a ByteArrayOutputStream
	 * @see ByteArrayOutputStream
	 */
	public static void decode(InputStream b64CharStream, OutputStream byteStream) throws IOException {
	 ...
	 * @param b64CharStream
	 *            In-param to receive result, usually a ByteArrayOutputStream,
	 *            then convert to String using String(byte[], Charset).
	 * @see ByteArrayOutputStream
	 */
	public static void encode(InputStream byteStream, OutputStream b64CharStream) throws IOException {
		...
		while ((c = s.read()) != -1) {
			...
			switch (st) {
			case 0:
				t.write(toB64Char((b & 0x00FC) >> 2));
				break;
			case 1:
				t.write(toB64Char((b & 0x03F0) >> 4));
				break;
			case 2:
				t.write(toB64Char((b & 0x0FC0) >> 6));
				t.write(toB64Char((b & 0x003F)));
				break;
			}
	...
	public static void main(String[] args) {
		try {
			...
			for (int i = 0; i < s.length; i++)
				System.out.print(String.format("%02x", s[i] & 0xff));
			System.out.println();
			...
			for (int i = 0; i < d.length; i++)
				System.out.print(String.format("%02x", d[i] & 0xff));
			System.out.println();
		} catch (Exception ex) {
			ex.printStackTrace();
		}
