...
/**
 * Transient tool-object to load certificate and generate ssl-context Code
 * adapted from nyafx/crypto. Thread-unsafe. This toolkit provided an ease way
 * to load certificate meterial from classpath
 */
public class AppCertificateLoader {
	// CONSTANTS
	...
	/**
	 * Trusts To store server certificates
	 */
	public KeyStore trusts;
	/**
	 * Iddentifiers To store our own certificates
	 */
	public KeyStore identification;

	public String passphrase;

	public AppCertificateLoader() {
		return;
	}

	public AppCertificateLoader(KeyStore trusts, KeyStore identification) {
		this();
	...
	// IMPORT
	/**
	 * Load trusts from .crt file. Existing <code>trusts</code> will be dropped.
	 */
	public AppCertificateLoader loadTrusts(String... resourcePaths) {
		try {
	...
	/**
	 * Load identification from .p12 file. Existing <code>identification</code>
	 * will be dropped.
	 * 
	 * @param passphrase
	 *            Passphrase to decrypt the cert, mch_id if certificate is
	 *            acquired from pay.weixin.qq.com .
	 */
	public AppCertificateLoader loadIdentification(String resourcePath, String passphrase) {
		try {
			InputStream stream = this.getClass().getResourceAsStream(resourcePath);

			if (stream == null)
				throw (new MissingResourceException("Load identification failed.", AppCertificateLoader.class.getName(),
						resourcePath));
	...
	public AppCertificateLoader config(Properties conf) {
	...
	public static void main(String[] args) {
		try {
			InputStream stream = AppCertificateLoader.class.getResourceAsStream("/wxpay.properties");

			Properties conf = new Properties();
			conf.load(stream);

			AppCertificateLoader cl = new AppCertificateLoader().config(conf);
