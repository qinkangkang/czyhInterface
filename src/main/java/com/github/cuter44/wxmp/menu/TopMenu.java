...
public class TopMenu extends JSONArray {
	...
	/**
	 * @param e
	 *            must have name set before add.
	 */
	public TopMenu add(TopMenuElement e) throws ArrayIndexOutOfBoundsException, IllegalArgumentException {
		if (this.size() >= TOP_MENU_SIZE_CONSTRAINT)
			throw (new ArrayIndexOutOfBoundsException(
					"Top menu element limit " + TOP_MENU_SIZE_CONSTRAINT + " excessed."));

		try {
			if (e.getName().getBytes("UTF-8").length > ELEMENT_NAME_BYTE_SIZE_CONSTRAINT)
				throw (new IllegalArgumentException(
						"Element name length limit " + ELEMENT_NAME_BYTE_SIZE_CONSTRAINT + " bytes excessed."));
