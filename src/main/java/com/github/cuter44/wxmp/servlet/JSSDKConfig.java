 ...
 *
 * 为 JS-SDK config 提供签名支持, 工作模式参见 <a href=
...
public class JSSDKConfig extends HttpServlet {
	...
	/**
	 * 读取配置文件 覆盖此方法可以删除对配置文件的访问, 此情况下 getAppid(), getJSSDKTicket() 均需要自行实现.
	 */
	@Override
	public void init() {
	...
	/**
	 * 提供 ticket 参数 servlet 从此方法取得必需参数 ticket, 覆盖此方法可以自定义 ticket 的来源. 默认实现从
	 * WxFactory.getDefaultInstance() 获取 TokenKeeper
	 * 
	 * @see com.github.cuter44.wxpay.TokenKeeper
	 */
	public String getTicket(String appid) {
		return (this.tokenKeeper.getJSSDKTicket());
	}
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
		...
		nonceStr = nonceStr != null ? nonceStr : this.crypto.byteToHex(this.crypto.randomBytes(8));
		...
		String signature = this.crypto.byteToHex(this.crypto.SHA1Digest(ub.toString().getBytes("utf-8")));
