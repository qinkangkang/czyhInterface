...
public class WxmpFactory {
	 ...
	 * 则需要以此方法手动初始化 TokenKeeper. 带参的构造方法会尝试在配置完成后调用这个方法, 如果传参 包含 appid 和 secret
	 * 则无需再手动配置. TokenKeeper 为所有请求及默认实现的 servlet 保持 access token 和 jsapi ticket
	 * 的缓存及刷新服务. 必需在 servlet 初始化前完成对这个方法的调用, 并且在 WxmpFactory 生命周期中只调用一次.
	 */
	 ...
	 * Auto invoked by <code>new WxmpFactory(Properties conf)</code> and
	 * <code>new WxmpFactory(String resource)</code>.
	 */
	 ...
	 * <code>new WxmpFactory(Properties conf)</code> and
	 * <code>new WxmpFactory(String resource)</code>.
	 */
	public WxmpFactory initRequestBase() {
		WxmpRequestBase.configDefaultHC(new CertificateLoader().config(this.conf).asSSLContext());
	 ...
	 * <code>resource</code>.
	 */
	public WxmpFactory(String resource) throws MissingResourceException {
		this();

		try {
			this.conf.load(new InputStreamReader(WxmpFactory.class.getResourceAsStream(resource), "utf-8"));
		} catch (Exception ex) {
			MissingResourceException mrex = new MissingResourceException("Failed to load conf resource.",
					WxmpFactory.class.getName(), resource);
			mrex.initCause(ex);
	 ...
	 * WxmpFactory in your application, DO NOT use this method.
	 */
	 ...
	 * 
	 * @param clazz
	 *            Subclass of WxmpRequestBase, must implement constructor
	 *            `clazz(Properties)`.
	 */
	...
	public TokenClientCredential newTokenClientCredential() {
		return (new TokenClientCredential(new Properties(this.conf)));
	}

	public TokenClientCredential newTokenClientCredential(Properties conf) {
		return (new TokenClientCredential(buildConf(conf, this.conf)));
	}
