...
public class GetBrandWCPayRequest extends WxpayRequestBase {
	...
	// KEYS
	public static final List<String> KEYS_PARAM_NAME = Arrays.asList("appId", "nonceStr", "package", "signType",
			"timeStamp");
	...
	public GetBrandWCPayRequest(Properties prop) {
		...
		if (super.getProperty(KEY_GBWCPR_APP_ID) == null)
			this.setAppId(super.getProperty(KEY_APPID));

		if (super.getProperty(KEY_GBWCPR_NONCE_STR) == null)
			this.setNonceStr(super.getProperty(KEY_NONCE_STR));

		if (super.getProperty(KEY_PACKAGE) == null)
			this.setPackage(super.getProperty(KEY_PREPAY_ID));
	...
	// SIGN
	@Override
	public GetBrandWCPayRequest sign() throws UnsupportedEncodingException {
		super.sign(KEYS_PARAM_NAME);

		super.setProperty(KEY_PAY_SIGN, super.getProperty(KEY_SIGN));
	...
	public String toJSON() {
		...
		for (String key : KEYS_PARAM_NAME)
			json.put(key, super.getProperty(key));
	...
	public WxpayRequestBase setTimeStamp(Date timeStamp) {
		super.setProperty(KEY_TIME_STAMP, Long.toString(timeStamp.getTime() / 1000L));
	 ...
	 * 
	 * @param paxkage
	 *            prepay_id, part after the equal
	 */
